...
# 打开 a.py 并设置变量 x 的值
with open("SCRIPT/select_ver1.py", "r", encoding="utf-8") as file:
...
# 写回 a.py
with open("SCRIPT/select_ver1.py", "w", encoding="utf-8") as file:
...
with zipfile.ZipFile(f"SUCCESS_PACK\{user_input}stiei_spider.rar", "w") as zipf:
    for file_name in ["SCRIPT/geckodriver.exe", "SCRIPT/Firefox Setup 121.0.1.exe", "dist/select_ver1.exe"]:
